%% Script plotTwoParallelLink()
% Goal:
% graphs the number of retransmissions for a Two Parallel-Link Network
...
for i = 1:5
    ...
    for j = 1:99
        p = p_values(j);
        sim_results(j) = runTwoParallelLinkSim(K,j./100,N);
    end
    ...
    title(sprintf('Two-Parallel Link Network: Results for K = %d', K));
    set(gca, 'YScale', 'log');
end
...
xlabel('Probability of failure (p)');
ylabel('Number of transmissions (log scale)');
title('Two-Parallel Link Network: Results for K = 1, 5, 15, 50, 100');
set(gca, 'YScale', 'log');